...
const PlywoodSlug = () => {
  ...
  const totalPaid = data?.plywood?.[0]?.paid?.reduce(
  ...
  return (
    <div className="mt-24 py-4 px-2 md:p-4 max-w-4xl mx-auto">
      ...
      <form
        ...
        className="bg-white px-2 py-4 md:p-6 rounded-xl shadow-md mb-8"
      >
            ...
            <p className="mt-1 text-2xl font-semibold text-red-600">
              ...
              {(
                (data.plywood?.[0]?.plywoodDetails?.reduce(
                  (total, item) => total + item.quantity * item.ratePerSheet,
                  0
                ) || 0) - totalPaid
              ).toLocaleString()}
            </p>
